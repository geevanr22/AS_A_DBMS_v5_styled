...
class FilingSystem(QWidget):
    ...
    def radio_button_dc_mm(self):# radio buttons for directors circular and members meeting
        ...
        self.horizontal_radio_button_dc_1 = QRadioButton("DR")
        self.horizontal_radio_button_dc_1.clicked.connect(self.horizontal_radio_button_dc_clicked) # this retrieves the selected input
        self.horizontal_radio_button_dc_2 = QRadioButton("BOD")
        self.horizontal_radio_button_dc_2.clicked.connect(self.horizontal_radio_button_dc_clicked) # this retrieves the selected input

        self.horizontal_radio_button_mm_1 = QRadioButton("AGM")
        self.horizontal_radio_button_mm_1.clicked.connect(self.horizontal_radio_button_mm_clicked) # this retrieves the selected input
        self.horizontal_radio_button_mm_2 = QRadioButton("EGM")
        self.horizontal_radio_button_mm_2.clicked.connect(self.horizontal_radio_button_mm_clicked) # this retrieves the selected input


        # Radio Button Grouping Director Circular (DR and BOD buttons)
        self.radio_button_group_dc = QButtonGroup(self)
        self.radio_button_group_dc.addButton(self.horizontal_radio_button_dc_1)
        self.radio_button_group_dc.addButton(self.horizontal_radio_button_dc_2)

        # Radio Button Grouping Members Meeting (AGM and EGM buttons)
        self.radio_button_group_mm = QButtonGroup(self)
        self.radio_button_group_mm.addButton(self.horizontal_radio_button_mm_1)
        self.radio_button_group_mm.addButton(self.horizontal_radio_button_mm_2)

    def horizontal_radio_button_dc_clicked(self):
        self.company = self.searchComboBox.currentText()
        self.combo_box_empty_or_not = self.comboBoxList # this is to check if the combo box something to be selected

        if self.combo_box_empty_or_not: # if there is a company at least, then the following code takes place (We can update the radio button option to the db)

            if self.horizontal_radio_button_dc_1.isChecked():
                # self.company = self.combo_box_list_company
                self.dr_or_bod = self.horizontal_radio_button_dc_1.text()

            elif self.horizontal_radio_button_dc_2.isChecked():
                # self.company = self.combo_box_list_company
                self.dr_or_bod = self.horizontal_radio_button_dc_2.text()
            else:
                pass

            try:
                print(self.dr_or_bod)
                self.c.execute(f"""UPDATE {self.tableName} SET Dir_or_BOD = '{self.dr_or_bod}' WHERE CoName ='{self.company}' """)
                self.conn.commit()
                print('DB Updated with DR or BOD')
            except:
                customMessage('Not able to update db with Directors Circular selection')

        else:
            customMessage('You need to select a company from the combo box before any selection')

    def horizontal_radio_button_mm_clicked(self):
        self.company = self.searchComboBox.currentText()
        ...
        if self.combo_box_empty_or_not: # if there is a company at least, then the following code takes place (We can update the radio button option to the db)
            ...
            if self.horizontal_radio_button_mm_1.isChecked():
                # self.company = self.combo_box_list_company
                self.agm_or_egm = self.horizontal_radio_button_mm_1.text()


            elif self.horizontal_radio_button_mm_2.isChecked():
                # self.company = self.combo_box_list_company
                self.agm_or_egm = self.horizontal_radio_button_mm_2.text()
    ...
    def layout(self):
        ...
        self.horizontal_layout_radio_button = QHBoxLayout()
        # self.mainLayoutTab2.addSpacing(10)
        self.horizontal_layout_radio_button.addWidget(self.horizontal_radio_button_dc_1)
        self.horizontal_layout_radio_button.addWidget(self.horizontal_radio_button_dc_2)
        self.horizontal_layout_radio_button.addWidget(self.horizontal_radio_button_mm_1)
        self.horizontal_layout_radio_button.addWidget(self.horizontal_radio_button_mm_2)
        self.mainLayoutTab2.addLayout(self.horizontal_layout_radio_button)
    ...
    def upload_directors_and_members_selection_function(self):
        print(self.dr_or_bod)
        print(self.agm_or_egm)
        print(self.combo_box_co_name_selection)

        # try:
        #     self.cur.execute(f"""UPDATE {self.tb_name} SET DR_or_BOD = '{self.dr_or_bod}' WHERE Company_Name ='{self.combo_box_co_name_selection}' """)
        #     self.conn.commit()
        #     print('DB Updated with DR or BOD')
        # except:
        ...
        #
        # try:
        #     self.cur.execute(f"""UPDATE {self.tb_name} SET AGM_or_EGM = {self.agm_or_egm} WHERE Company_Name = {self.combo_box_co_name_selection}""")
        #     self.conn.commit()
        #     print('DB Updated with AGM or EGM')
        # except:
        #     print('Not able to update db with Members Meeting selection')
