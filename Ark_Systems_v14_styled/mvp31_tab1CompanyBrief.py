...
from stackedFilingSystemMVP31 import StackedFilingSystem
from mvp31_dbTableAndInputClass import DB_Table_Input
from mvp31_mainUIClass_Version2 import MainUI
...
class Tab1(QWidget):
    ...
    # we have switched this off(submitRecordsButton2), feel free to delete once the new one(submitRecordsButton) is working
    def submitRecordsButton2(self):
        """ This is the main button that validates all inputs, creates the database
        connects to the table, uploads the data collected (this is done
        in the self.checkDuplicateCoName function) to the database and opens
        the filing system window"""



        self.dataClean() # This function from below cleans all inputs before being presented here to be uploaded to the DB
        ...
        try:
            ...
            if self.FileNo != "" and self.CoName != "":
                if self.file_no_validated == 'Validated':
                    ...
                    if self.check_if_co_name_not_duplicate == 'Validated':
                        print('Company Name is unique')
                        if self.check_if_incorporated_date_changed == 'Validated':
                            print('Date inputed')
                            self.generateFolders()
                            self.dataUploadtoDataBase()
                        else:
                        ...
                        print('Check for duplicate company name did not pass')
                else:
    ...
    def openFilingSystemButton(self):
        ...
        self.filingSystem = StackedFilingSystem()
    ...
    # def typeRadioButtonClicked(self):
    #     if self.typeRadioButton_1.isChecked():
    #         self.CoType = self.typeRadioButton_1.text()
    #
    #     elif self.typeRadioButton_2.isChecked():
    #         self.CoType = self.typeRadioButton_2.text()
    #
    #     else:
    ...
    def dataUploadtoDataBase(self):
        ...
        self.c.execute(f"INSERT INTO {self.tableName} ({self.field1}, {self.field2},{self.field3} ,{self.field4},"
                       ...
                       (self.FileNo, self.CoName, self.CoOldName, self.RegNO, self.DateOfChange,
                        ...
                        self.Director1, self.Director2, self.Secretaries1, self.Secretaries1AppointedDate,
